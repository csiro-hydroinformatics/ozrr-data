...
from ozrr.read import data_load, load_rr_series, get_catchment_id
...
def to_monoindex_series(x_multiindex: xr.DataArray):
...
def to_pd_series(x: xr.DataArray):
...
class OzDataProcessing:
    def __init__(self, root_dir, nc_filename=None) -> None:
    ...
    def load_daily_data(self, discard_cached_nc=False, lazy_loading=False):
        import json
...
class MaskTimeSeries:
    def __init__(self, observed_ts) -> None:
...
class OzDataProvider:
    """A high level facade to access 'ozdata' or its derived datasets"""
...
def load_aus_rr_data(
    aus_rr_root_dir=None,
    discard_cached_nc=False,
